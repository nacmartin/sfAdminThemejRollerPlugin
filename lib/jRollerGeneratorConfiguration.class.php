...
abstract class jRollerGeneratorConfiguration extends sfModelGeneratorConfiguration
{
  ...
  protected function compile()
  {
    ...
    foreach (array_keys($config['default']) as $field)
    {
      ...
      $this->configuration['list']['fields'][$field]   = new jRollerGeneratorConfigurationField($field, array_merge(array('label' => sfInflector::humanize(sfInflector::underscore($field))), $config['default'][$field], isset($config['list'][$field]) ? $config['list'][$field] : array()));
      $this->configuration['filter']['fields'][$field] = new jRollerGeneratorConfigurationField($field, array_merge($config['default'][$field], isset($config['filter'][$field]) ? $config['filter'][$field] : array()));
      $this->configuration['new']['fields'][$field]    = new jRollerGeneratorConfigurationField($field, array_merge($formConfig, isset($config['new'][$field]) ? $config['new'][$field] : array()));
      $this->configuration['edit']['fields'][$field]   = new jRollerGeneratorConfigurationField($field, array_merge($formConfig, isset($config['edit'][$field]) ? $config['edit'][$field] : array()));
    }

    // "virtual" fields for list
    foreach ($this->getListDisplay() as $field)
    {
      list($field, $flag) = jRollerGeneratorConfigurationField::splitFieldWithFlag($field);

      $this->configuration['list']['fields'][$field] = new jRollerGeneratorConfigurationField($field, array_merge(
    ...
    {
      list($name, $flag) = jRollerGeneratorConfigurationField::splitFieldWithFlag($name);
  ...
  protected function parseVariables($context, $key)
  {
    ...
    foreach ($matches[1] as $name)
    {
      ...
      {
        $this->configuration[$context]['fields'][$name] = new jRollerGeneratorConfigurationField($name, array_merge(
  ...
  public function getFormFilterFields(sfForm $form)
  {
    ...
    if ($this->getFilterDisplay())
    {
      ...
      foreach ($this->getFilterDisplay() as $name)
      {
        ...
        {
          $this->configuration['filter']['fields'][$name] = new jRollerGeneratorConfigurationField($name, array_merge(
            isset($config['default'][$name]) ? $config['default'][$name] : array(),
    ...
    {
      $fields[$name] = new jRollerGeneratorConfigurationField($name, array_merge(
        array('type' => 'Text'),
        isset($config['default'][$name]) ? $config['default'][$name] : array(),
        isset($config['filter'][$name]) ? $config['filter'][$name] : array(),
        array('is_real' => false)
